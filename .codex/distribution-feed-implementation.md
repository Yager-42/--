...
> - 本仓库按用户要求：不做本地验证（所以“验收用例”不会被勾选）

#### Phase 1（MVP：写扩散 + timeline + profile + 负反馈）

- [x] 新增 `PostPublishedEvent`（`nexus-types`）
- [x] 新增 `FeedFanoutConfig` + `FeedFanoutConsumer`（`nexus-trigger`）
...
- [x] 在线推：`FeedDistributionService.fanout` 作者无条件写入 inbox；粉丝仅对 `inboxExists(userId)=true` 的用户写入 inbox
- [x] 配置落地：`feed.rebuild.*` / `feed.inbox.ttlDays` / `feed.fanout.batchSize`（`application-dev.yml` 给出示例）

#### 验收/验证（可选；本次未执行）

- [ ] 发布内容 → 粉丝 timeline 能看到
- [ ] timeline 翻 3 页不重复/不漏
- [ ] 负反馈提交/撤销对可见性生效

#### 未完成（后续）

- [ ] Phase 3：推荐与排序
- [ ] 10.5.1 fanout 大任务切片（规模化）
...
- [ ] 热点探测 + L1 本地缓存：`JD HotKey` + Caffeine（短 TTL，只缓存 hot key；落地清单见 `.codex/interaction-like-pipeline-implementation.md` 的 2.2.6）
...
consume(PostPublishedEvent e):
  ...
  // 2) 计算 slice 数量并投递任务
  total = relationRepo.countRelationsByTarget(e.authorId, RELATION_FOLLOW)  // 粗略计数即可
...
如果你不信 `relationRepo.countRelationsByTarget` 的数据（真值以 `user_follower` 为准），那就给 `IFollowerDao` 增加一个 `countFollowers(userId)`（SQL：`COUNT(*) FROM user_follower WHERE user_id=?`），由 `RelationRepository` 暴露出来给 dispatcher 用。
