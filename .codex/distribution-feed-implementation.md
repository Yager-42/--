...
> 更新时间：2026-01-13  
> 结论：✅ Phase 1 已完成；✅ Phase 2 已完成（在线推 / 离线拉）。
...
- ✅ Redis 负反馈：IFeedNegativeFeedbackRepository + FeedNegativeFeedbackRepository（SET：postId + 内容类型 media_type）
...
已完成（Phase 2）：
- ✅ 在线判定：`IFeedTimelineRepository.inboxExists`（以 inbox key 是否存在定义在线）
- ✅ 原子重建：`IFeedTimelineRepository.replaceInbox`（lock + tmpKey + RENAME 覆盖 + NoMore + TTL）
- ✅ NoMore 哨兵：member=`__NOMORE__`；`pageInbox` 过滤 NoMore 且读侧刷新 TTL（EXPIRE）
- ✅ 离线拉：`FeedInboxRebuildService` + `FeedService.timeline` 首页 inbox miss 触发重建并返回近况
- ✅ 关注回源兜底：`RelationAdjacencyCachePort.listFollowing` 在缓存 key miss 或 relation 表缺失时，可从 `user_follower` 回源关注列表，避免重建空 inbox
- ✅ 在线推：`FeedDistributionService.fanout` 作者无条件写入 inbox；粉丝仅对 `inboxExists(userId)=true` 的用户写入 inbox
- ✅ 配置落地：`feed.rebuild.*` / `feed.inbox.ttlDays` / `feed.fanout.batchSize`（`application-dev.yml` 给出示例）

未完成（后续）：
- ⏳ Phase 3：推荐与排序（未实现）
- ⏳ 可选优化：fanout 大任务切片（10.5.1）、follow/unfollow 最小补偿（10.5.2）（未实现）

可改进点（不影响 Phase 2 交付）：
- MQ 消息序列化可统一为 JSON（目前沿用默认可运行方式，避免引入额外配置分叉）
- timeline 读侧可做批量 postId 负反馈过滤（减少 `SISMEMBER` 次数）；内容类型负反馈已按集合一次性读取
- feed fanout 可补齐 DLQ/重试与监控指标（类似 ContentSchedule 的 DLQ 思路）
- fanout 的 `inboxExists` 可做 Redis pipeline（减少 1:1 round-trip）
- 负反馈“内容类型”目前仅覆盖 `content_post.media_type`（纯文/图文/视频）；若要业务类目/标签维度，需要补齐类型表与发布写入链路
...
边界情况（必须写进实现里）：
- followings 为空：`replaceInbox(userId, [])`（仍会写 NoMore，避免反复 miss）
- 关注的人都没发过内容：同上（空 inbox 也要有 key，否则每次首页都会重建）
...
当单条 `PostPublishedEvent` 需要处理的在线粉丝很多时（例如在线粉丝 > 5 万），不要在一个 consumer 里跑完整 fanout：
- 新增 dispatcher：消费 `PostPublishedEvent` 后只负责扫描粉丝分页，并拆成多个 `FeedFanoutTask`（slice：offset+limit）
- 多个 worker 并行消费 `FeedFanoutTask`，失败重试只重试这一片，避免“从头再来”

#### 10.5.2 follow/unfollow 的最小补偿（让体验更像正常产品）

来源标注：《从小白到架构师(4): Feed 流系统实战》

- follow：如果 follower inbox key 存在（在线），可选回填 target 最近 K 条内容进 follower inbox（否则用户只会看到“关注之后的新内容”）
- unfollow：最简单是“不回收历史”（让 TTL 自然过期）；更强策略是 evict follower inbox，让其下次首页重建（更准但更重）
...
## 12. 实现者 Checklist（照着打勾就能交付）

### 12.1 Phase 1 必须完成（最小闭环）

- [ ] 新增 `PostPublishedEvent`（types）
- [ ] 改造 `ContentDispatchPort`：发布 MQ 消息
- [ ] 新增 `FeedFanoutConfig` + `FeedFanoutConsumer`（trigger）
- [ ] 新增 `IFeedDistributionService` + 实现（domain）
- [ ] 新增 `IFeedTimelineRepository` + Redis 实现 `FeedTimelineRepository`
- [ ] 新增 `IFeedNegativeFeedbackRepository` + Redis 实现
- [ ] 扩展 `IFollowerDao`/`FollowerMapper.xml` 支持分页列粉丝
- [ ] 扩展 `IRelationRepository` 暴露 `listFollowerIds`
- [ ] 扩展 `IContentPostDao`/`ContentPostMapper.xml`：
  - `selectByIds`
  - `selectByUserPage`
- [ ] 扩展 `IContentRepository`/`ContentRepository`：
  - `listPostsByIds`
  - `listUserPosts`
- [ ] 改造 `FeedService`：timeline/profile/负反馈走真实链路

### 12.2 最少验收 3 个场景（本地跑通）

1. 发布内容 → 粉丝 timeline 能看到
2. timeline 翻 3 页不重复/不漏
3. 负反馈提交/撤销对可见性生效

### 12.3 Phase 2 任务清单（最终方案 A：在线推 / 离线拉）

- [ ] 修改 `IFeedTimelineRepository`：
  - `boolean inboxExists(Long userId)`
  - `void replaceInbox(Long userId, List<FeedInboxEntryVO> entries)`（原子化重建）
  - `pageInbox`：过滤 NoMore + 读侧刷新 TTL（EXPIRE）
- [ ] 修改 `FeedTimelineRepository`：
  - NoMore 哨兵：`member="__NOMORE__"`（空 Timeline 也要有 key）
  - 重建原子性：lock + tmpKey + `RENAME` 覆盖正式 inbox
  - `nextCursor` 不能返回 NoMore
- [ ] 新增 `FeedInboxEntryVO`（domain valobj）
- [ ] 新增 `IFeedInboxRebuildService` + `FeedInboxRebuildService`
- [ ] 修改 `FeedService.timeline`：首页 + inbox miss → rebuild → pageInbox
- [ ] 修改 `FeedDistributionService.fanout`：只对 `inboxExists(followerId)` 的粉丝写 inbox（作者自己无条件写）
- [ ] 配置落地：`feed.rebuild.*`（含 `lockSeconds`）与 `feed.inbox.ttlDays`（建议 7）
- [ ] （可选）按 10.5.1 实现 fanout 切片任务（大规模在线粉丝）
- [ ] （可选）按 10.5.2 实现 follow/unfollow 最小补偿

### 12.4 Phase 2 验收点（必须通过）

1. 在线推生效：只写入 inbox key 存在的粉丝
2. 离线拉生效：离线用户回归拉 timeline 首页能触发重建并返回近况
3. 资源回收：超过 `feed.inbox.ttlDays` 未活跃的用户 inbox key 过期后不再接收 pushes
