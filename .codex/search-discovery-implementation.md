...
这样做的好处：Search 域只依赖已存在的 `post.published/post.deleted`（`social.feed`），不会被 USER/GROUP 的上游数据源与事件生产者卡住上线。
...
- `type` 参数仍然接受（HTTP 契约不变），但只做 POST 搜索：
  - 空/`ALL`/`POST`：按 `POST` 处理
  - `USER`/`GROUP`/其它：视为 `UNSUPPORTED`，返回空列表（items=[]），不抛错；facets 仅返回 `meta`：`{"meta":{"reason":"UNSUPPORTED_TYPE","normalizedType":"UNSUPPORTED","offset":0,"limit":20}}`
...
- `user_base`：用于补全 `authorNickname`（username），已存在（见 `project/nexus/docs/social_schema.sql`）
...
### 4.1 事件来源（只复用 social.feed）

本次索引更新只依赖已存在的 Post 事件：
...
#### 4.3.1 PostPublishedEvent → upsert / delete 的判定规则
...
- `createTimeMs = post.createTime`（null → event.publishTimeMs）
...
2. 将连续空白压缩为单个空格
3. 长度上限 64（超过截断）
4. 归一化后为空：直接返回空结果（items=[]；facets 仅返回 `meta`：`{\"meta\":{\"reason\":\"EMPTY_KEYWORD\",\"normalizedType\":\"<NORMALIZED_TYPE>\",\"offset\":<OFFSET>,\"limit\":<LIMIT>}}`）
...
- `offset`：默认 0；最小 0；最大 2000（超过按 2000 处理）
- `limit`：默认 20；最小 1；最大 50（超过按 50 处理）
- `postTypes`：可选；为空表示不过滤；最多 5 个（超过截断）
- `mediaType`：可选；为空表示不过滤；允许 0/1/2
- `timeRange.fromMs/toMs`：可选；0 表示不限制
- `category`：用于热搜归因；`AUTO`/空表示 `POST`；允许 `POST`/`ALL`（`ALL` 当成 `POST`；其它值也当成 `POST`）
- `includeFacets`：默认 true
...
> 注意：我们不做 deep pagination。`offset` 最大 2000 是硬限制（见 5.2）。
...
- `RELEVANT`：默认 ES `_score desc`，再 `createTimeMs desc`
- `LATEST`：`createTimeMs desc`，再 `_score desc`
...
`facets`：JSON 字符串（见 5.5）；当 `includeFacets=false` 时，仍然返回带 `meta` 的 JSON（但不执行 ES aggregations）。
...
- `meta` 必须永远返回（即使 includeFacets=false 也要返回）：
  - `reason` 允许值（写死）：`OK` / `EMPTY_KEYWORD` / `UNSUPPORTED_TYPE` / `ILLEGAL_FILTERS`
  - `normalizedType`：5.1 归一化后的 type（含 `UNSUPPORTED`）
  - `offset/limit`：5.2 归一化后的分页参数
- 仅当 `filters.includeFacets != false` 时执行 ES aggregations；否则只返回 `meta` + 空对象
- aggs 口径（写死）：
  - `mediaType`：terms on `mediaType`
  - `postTypes`：terms on `postTypes`
...
当 `userId != null` 且 keyword 非空且 type != `UNSUPPORTED` 时：
...
2. 若 keyword 非空：
   ...
   - 逐个按“前缀匹配”过滤（区分大小写：不区分；按归一化后的 keyword 进行）
   - 取前 10 个返回
...
2. `project/nexus/nexus-trigger/src/main/java/cn/nexus/trigger/mq/consumer/SearchIndexConsumer.java`
   - `@RabbitListener(queues = ...)` 监听两条队列（见 4.2）
   - 处理规则按 4.3（回表 → upsert/delete）
   - 失败：抛 `AmqpRejectAndDontRequeueException` 进入 DLQ（坏数据不要悄悄吞）
...
- DLQ 深度 > 0：`search.post.published.dlx.queue` / `search.post.deleted.dlx.queue`
