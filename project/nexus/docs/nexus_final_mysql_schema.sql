...
CREATE TABLE IF NOT EXISTS `interaction_reaction` (
  ...
  `reaction_type` VARCHAR(16) NOT NULL COMMENT 'LIKE等',
  `user_id` BIGINT NOT NULL,
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`target_type`, `target_id`, `reaction_type`, `user_id`),
  KEY `idx_reaction_target_time_user` (`target_type`, `target_id`, `reaction_type`, `update_time`, `user_id`),
  KEY `idx_reaction_user_target` (`target_type`, `reaction_type`, `user_id`, `target_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='互动态势事实表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='互动态势计数表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='计数增量幂等去重表';
