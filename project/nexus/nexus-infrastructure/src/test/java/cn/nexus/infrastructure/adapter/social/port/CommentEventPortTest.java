...
@SpringJUnitConfig(CommentEventPortTest.TestConfig.class)
class CommentEventPortTest {

    @Autowired
    private CommentEventPort port;
    ...
    void publish_shouldSaveOutboxThroughReliableMqPublishAspect() {
        ...
        event.setEventId("comment-event-1");

        port.publish(event);

        assertTrue(AopUtils.isAopProxy(port));
        verify(reliableMqOutboxService).save("comment-event-1", "social.interaction", "comment.created", event);
    }

    @Configuration
    @org.springframework.context.annotation.EnableAspectJAutoProxy(proxyTargetClass = true)
    @Import(CommentEventPort.class)
