...
> 约定：`user_id` 不作为 Body/Query 参数传入；由网关/登录态注入请求头 `X-User-Id`，服务端从请求上下文读取。  
> 备注：本仓库实际路由前缀为 `/api/v1`（见 `InteractionController`），下表用 `/v1` 作为简写。
...
| **获取点赞状态(单条)** | GET | `/v1/interact/reaction/state` | `target_id`, `target_type` (POST/COMMENT) | `like_count`, `liked_by_me` |
| **获取点赞状态(批量)** | POST | `/v1/interact/reaction/batchState` | `targets` (List: `{target_id,target_type}`) | `items` (List: `{target_id,target_type,like_count,liked_by_me}`) |
...
| **获取通知列表** | GET | `/v1/notification/list` | `cursor` | `notifications` (List) |
