...
### B4. 搜索建议 / 推荐上游结果 / 外部结果读缓存（候选池，不是直接实现契约）
...
- 本节只定义“值不值得做、应该放哪一层、失败时怎么降级”这类方向约束
- 本节**没有**给出 owner、key、TTL、失效矩阵、脏数据语义，所以**不允许**直接按这一节编码
- 只有补完独立子文档，并满足第十章 Phase 2 准入门槛后，才能从候选池转成执行契约

### B5. KV 内容缓存（候选池，仅在读频率真实够高且独立子文档补完时做）
...
- 如果没有真实 QPS、回源耗时、热点分布这三类证据，就默认**不做**
- 本节当前只保留准入规则，不是直接实现契约
- 只有补完独立子文档，并满足第十章 Phase 2 准入门槛后，才能进入开发
...
- 当前代码里 `RelationMapper.xml` 已经存在 `pageActiveByTarget` 的 keyset SQL 形态
- 但 `docs/social_schema.sql` 里还**没有明确声明**覆盖 `target_id + relation_type + status + create_time + source_id` 的目标侧复合索引
- 所以这条现在不是“实现时自己选一个差不多的索引”，而是**上线前必须显式确认索引存在**；没确认，就按本条硬阻塞处理
...
6. 基础稳定后，只允许继续进入 `B1/B2/B3` 这类已补完契约的新增缓存建设；`B4/B5`（搜索建议、推荐上游结果、KV 内容缓存）当前仍是候选池，必须先补独立子文档再实现
...
4. 如果继续走 `user_relation.pageActiveByTarget`，必须先确认存在覆盖 `target_id + relation_type + status + create_time + source_id` 的目标侧复合索引；没有这个前提，不准把它当“可直接实现路径”
5. 热路径禁止高频 `count(*)`
6. 允许的唯一小优化，是单次请求作用域内显式结果复用
...
前文 B 线里除 `B1/B2/B3` 外的剩余点位保留，但这里只能作为候选池。任何一个 B 线新缓存点要真正进入开发，必须先补齐下面 6 项：
