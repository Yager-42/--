...
| 字段 | 含义 | 示例 |
| --- | --- | --- |
| `eventId` | 本次事件唯一 ID（链路追踪） | `evt_123` |
| `userId` | 当前用户 | `10001` |
...
| `DEGRADE_VISIBILITY` | 降权/影子禁言/隔离发布 | 主要影响分发域；可用 `visibility=SHADOWBAN/QUARANTINE` 区分 |
...
| `FOLLOW/LIKE` 等高频互动 | 可选（建议只做频控） | 否 | 只做频控/刷量检测；LLM 不参与 |
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `actionType` | 是 | `PUBLISH_POST` / `COMMENT_CREATE` / `FOLLOW` / `LOGIN` ... |
